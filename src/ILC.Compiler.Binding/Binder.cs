...
{
    public BindingResult Bind(SyntaxTree syntaxTree, BindingProfiler? profiler = null)
    {
        var diagnostics = new DiagnosticBag();
        using (Profile(profiler, "CopySyntaxDiagnostics"))
        {
        ...
        using (Profile(profiler, "CollectDeclaredTypeShells"))
        {
        ...
        using (Profile(profiler, "BindProvisionalDeclaredTypes"))
        {
            foreach (var member in syntaxTree.Root.Members)
            {
                switch (member)
        ...
        using (Profile(profiler, "BindDeclaredTypes"))
        {
            foreach (var member in syntaxTree.Root.Members)
            {
                switch (member)
        ...
        using (Profile(profiler, "CollectConstructedGenericTypes"))
        {
            foreach (var constructedType in CollectConstructedGenericTypes(syntaxTree, declaredTypes, profiler))
            {
        ...
        using (Profile(profiler, "AddConstructedGenericClosure"))
        {
            AddConstructedGenericClosure(declaredTypes);
        }

        MethodSymbol[] knownMethods;
        FieldSymbol[] knownFields;
        ConstantSymbol[] knownConstants;
        PropertySymbol[] knownProperties;
        using (Profile(profiler, "CollectKnownSymbols"))
        {
            knownMethods = declaredTypes
                .OfType<NamedTypeSymbol>()
                .SelectMany(type => type.Methods)
                .ToArray();
            knownFields = declaredTypes
                .OfType<NamedTypeSymbol>()
                .SelectMany(type => type.Fields)
                .ToArray();
            knownConstants = declaredTypes
                .OfType<NamedTypeSymbol>()
                .SelectMany(type => type.Constants)
                .ToArray();
            knownProperties = declaredTypes
                .OfType<NamedTypeSymbol>()
                .SelectMany(type => type.Properties)
                .ToArray();
        }

        using (Profile(profiler, "BindTopLevelMembers"))
        {
            foreach (var member in syntaxTree.Root.Members)
            {
        ...
        using (Profile(profiler, "SynthesizeTopLevelMain"))
        {
        ...
        SyntheticLambdaArtifacts lambdaArtifacts;
        var methodQueryScopeCache = new Dictionary<object, Dictionary<string, TypeSymbol>>(ReferenceEqualityComparer.Instance);
        using (Profile(profiler, "CollectSyntheticLambdaArtifacts"))
        {
            lambdaArtifacts = CollectSyntheticLambdaArtifacts(
        ...
        SyntheticProjectorArtifacts projectorArtifacts;
        using (Profile(profiler, "CollectSyntheticProjectorArtifacts"))
        {
            projectorArtifacts = CollectSyntheticProjectorArtifacts(
                syntaxTree.Root.Members,
                declaredTypes,
        ...
        using (Profile(profiler, "ValidateSemantics"))
        {
            ValidateSemantics(syntaxTree.Root.Members, globals, topLevelConstants, declaredTypes, allMethods, knownFields, allKnownConstants, knownProperties, diagnostics, profiler);
        }

        MethodSymbol[] explicitEntryPoints;
        ...
        using (Profile(profiler, "CreateCompilationSymbol"))
        {
            symbol = new CompilationUnitSymbol(
    ...
    private static BindingProfiler.ProfileScope Profile(BindingProfiler? profiler, string name) =>
        profiler?.Enter(name) ?? default;
}
